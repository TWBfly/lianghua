"""
Fast Direct Verification for Cotton (CF_IDX) 5m ML+PPO Strategy
...
def engineer_cf_features(df_raw):
    df = df_raw.copy()
...
def run_cf_ml_and_simulate(df_feat, horizon=40, target_atr=4.5, sl_label=1.0, prob_th=0.24, sl_atr=0.8, be_atr=1.3, lock1_atr=2.2, lock2_atr=3.6, trail_dist=1.4, max_bars=50):
    ...
    for i in range(len(df_sim)):
        ...
        if pos != 0:
            ...
            if pnl_atr >= lock2_atr:
                ...
                # 锁定中波段 (至少锁定 +1.2 ATR)
                if pos == 1: sl_p = max(sl_p, entry_p + 1.2 * catr)
                else: sl_p = min(sl_p, entry_p - 1.2 * catr)
            elif pnl_atr >= be_atr:
                # 保本
            ...
            if hit_sl or timeout:
                ...
                trades.append({
                    "pnl": realized - fee, "exit_p": exit_p, "bars": h_bars, "type": "SL" if hit_sl else "TIMEOUT"
                })
                pos = 0
...
def main():
    ...
    df_feat = engineer_cf_features(df_raw)

    print("\n🔍 正在测试不同组合方案...")
    tests = [
        # (horizon, target_atr, sl_label, prob_th, sl_atr, be_atr, lock1_atr, lock2_atr, trail_dist, max_bars)
        (36, 4.2, 0.9, 0.25, 0.7, 1.2, 2.0, 3.2, 1.2, 45),
        (40, 4.5, 0.8, 0.25, 0.7, 1.2, 2.2, 3.5, 1.2, 50),
        (40, 4.5, 0.8, 0.26, 0.8, 1.3, 2.2, 3.6, 1.4, 50),
        (48, 5.0, 0.8, 0.26, 0.8, 1.4, 2.4, 3.8, 1.4, 60),
        (48, 4.8, 0.8, 0.25, 0.75, 1.2, 2.2, 3.5, 1.3, 55),
        (30, 3.8, 0.8, 0.26, 0.7, 1.1, 1.8, 2.8, 1.1, 40),
        (40, 4.2, 0.8, 0.27, 0.7, 1.2, 2.0, 3.0, 1.2, 45),
    ]

    for idx, params in enumerate(tests, 1):
        res = run_cf_ml_and_simulate(df_feat, *params)
        if res:
            print(f"方案 {idx}: 参数={params}")
            print(f"   👉 交易数: {res['trades']}, 胜率: {res['win_rate']:.2f}%, 盈亏比(PF): {res['profit_factor']:.2f}, 单笔盈亏比: {res['payoff_ratio']:.2f}, 净利润: {res['total_pnl']:+,.2f}元")
            print(f"      平均盈利: {res['avg_win']:.2f}元, 平均亏损: {res['avg_loss']:.2f}元\n")
