"""
Refine 167-trade Cotton system to boost Profit Factor while maintaining trades >= 80 and Win Rate >= 51%
"""
...
df = df[(df["volume"] > 0) & (df["close"] > 0) & (df["high"] >= df["low"])].sort_values("datetime").reset_index(drop=True)
...
feature_cols = ["fast_trend", "slow_trend", "accel_5m", "squeeze_5m", "vol_burst_5m", "donchian_dist_5m", "oi_flow_5m", "rsi_14_5m"]
...
while current_idx < n_samples:
    ...
    clf_l = lgb.LGBMClassifier(n_estimators=60, learning_rate=0.03, max_depth=3, num_leaves=7, min_child_samples=30, random_state=42, verbose=-1, n_jobs=2)
    clf_s = lgb.LGBMClassifier(n_estimators=60, learning_rate=0.03, max_depth=3, num_leaves=7, min_child_samples=30, random_state=42, verbose=-1, n_jobs=2)
...
for p_th in [0.22, 0.23, 0.24]:
    for sl_atr in [0.9, 1.0, 1.1, 1.2]:
        for be_atr in [1.2, 1.4, 1.6, 1.8]:
            for lock_atr in [1.6, 1.8, 2.0, 2.2]:
                for trail_dist in [1.8, 2.0, 2.2, 2.5]:
                    for max_bars in [35, 45, 60]:
                        ...
                        for i in range(len(df_sim)):
                            ...
                            if pos == 0:
                                ...
                                if long_cond:
                                    pos = 1; lots = max_lots; entry_p = cp + 5.0; sl_p = entry_p - sl_atr * catr; peak_p = cp; entry_i = i; half_locked = False
                                elif short_cond:
                                    pos = -1; lots = max_lots; entry_p = cp - 5.0; sl_p = entry_p + sl_atr * catr; trough_p = cp; entry_i = i; half_locked = False
                        ...
                        if len(trades) >= 70:
                            ...
                            if wr >= 51.0 and net > 0:
                                best_res.append({
                                    "trades": len(tr), "wr": wr, "pf": pf, "net": net,
                                    "p_th": p_th, "sl_atr": sl_atr, "be_atr": be_atr,
                                    "lock_atr": lock_atr, "trail_dist": trail_dist, "max_bars": max_bars
                                })

best_res = sorted(best_res, key=lambda x: (x["wr"], x["pf"], x["trades"]), reverse=True)
print(f"Total matching configs: {len(best_res)}")
for idx, b in enumerate(best_res[:10], 1):
    print(f"Option {idx}: 交易数={b['trades']}, 胜率={b['wr']:.2f}%, 盈亏比={b['pf']:.2f}, 净利润={b['net']:+,.2f}元")
    print(f"         p_th={b['p_th']}, sl_atr={b['sl_atr']}, be_atr={b['be_atr']}, lock_atr={b['lock_atr']}, trail={b['trail_dist']}, max_bars={b['max_bars']}")
