"""
Cotton (CF_IDX) 5m Precision Optimizer
Goal: Win Rate >= 51%, Profit Factor (盈亏比) >= 3.0
"""
...
def run_targeted_search():
    ...
    # 选取几种典型的高胜率/大波段标签
    for horizon, target_atr, sl_label in [(36, 4.0, 1.0), (48, 4.5, 1.0), (48, 5.0, 1.2), (60, 5.0, 1.2)]:
                                ...
                                for max_bars in [35, 45, 60]:
                                    ...
                                    if len(trades) >= 15:
                                        ...
                                        cfg_cand = {
                                            ...
                                            "lock_atr": lock_atr, "trail_atr": trail_atr, "lock_pct": lock_pct,
    ...
    print(f"Searched finished. Evaluated {len(candidates)} configurations.")

    # 寻找最佳参数：胜率 >= 51%, 盈亏比尽可能高, 净利润 > 0
    high_wr = [c for c in candidates if c["wr"] >= 51.0 and c["net"] > 0]
    print(f"Found {len(high_wr)} configurations with Win Rate >= 51% and Net Profit > 0.")

    if high_wr:
        high_wr = sorted(high_wr, key=lambda x: (x["pf"], x["wr"], x["net"]), reverse=True)
        for i, q in enumerate(high_wr[:15], 1):
            print(f"Rank {i}: 胜率={q['wr']:.2f}%, 盈亏比(PF)={q['pf']:.2f}, 单笔盈亏比={q['payoff']:.2f}, 净利={q['net']:+,.2f}元, 交易数={q['trades']}")
            print(f"        {q['cfg']}")
