"""
Cotton (CF_IDX) 5m Balanced Sample Search (Trades >= 30~80)
"""
...
def search_balanced():
    ...
    for horizon in [20, 24, 30]:
        for target_atr in [2.8, 3.2, 3.6]:
            for sl_label in [0.8, 1.0]:
                ...
                for p_th in [0.24, 0.25, 0.26, 0.27]:
                    for sl_atr in [0.8, 0.9]:
                        for be_atr in [1.0, 1.2]:
                            for lock_atr in [2.0, 2.4]:
                                for trail_dist in [1.8, 2.2]:
                                    for max_bars in [25, 35]:
                                        pos = 0
                                        entry_p = 0.0
                                        sl_p = 0.0
                                        entry_i = 0
    ...
    print(f"✅ 统计充分样本池 (交易数 >= 30 笔 且 胜率 >= 50% 且 净利 > 0): {len(candidates)} 组")
    candidates = sorted(candidates, key=lambda x: (x["wr"] >= 51.0, x["trades"], x["pf"]), reverse=True)
