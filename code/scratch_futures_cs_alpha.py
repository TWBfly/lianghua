"""
Cross-Sectional Multi-Commodity Qlib Alpha Spread on 15m Futures
"""
import pandas as pd
import numpy as np
import sqlite3
...
clean_df = df_all.dropna(subset=feature_cols + ["label"]).copy()
...
for i, t in enumerate(times):
    ...
    # Scheduled Rebalance (Every 16 bars = 4 hours)
    ...
    if is_rebalance_bar and len(current_scores) >= (top_k * 2 + 4):
        ...
        # Mark total equity & compute allocatable capital per leg
        ...
        # Allocate 30% margin total (15% for longs, 15% for shorts, so gross leverage is ~1.5x, strictly safe!)
        alloc_margin_per_pos = (total_equity * 0.15) / max(1, top_k)
