"""
Audit and debug exact Qlib Futures 15M backtest ledger and drawdowns
"""
import pandas as pd
import numpy as np
import sqlite3
...
# Build daily records correctly by grouping on calendar date
daily_records = []
dates = sorted(df_bars["date"].unique())
for idx, d in enumerate(dates):
    d_bars = df_bars[df_bars["date"] == d]
    start_eq = daily_records[-1]["end_equity"] if daily_records else initial_cash
...
true_max_dd_pct = float(df_bars["drawdown"].max() * 100.0)

print("=" * 65)
print(f"Total Trades:           {len(closed_trades)} 笔")
...
print(f"Max Drawdown (Daily):   {perf.get('max_drawdown_pct', 0.0):.2f} %")
print(f"Max Drawdown (Bar 15m): {true_max_dd_pct:.2f} %")
print("=" * 65)
