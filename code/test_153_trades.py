"""
Verify the 153-trade Cotton configuration
"""
...
def test_153():
    ...
    print("=" * 80)
    print(f"Cotton 5m Rich Sample Backtest:")
    print(f"  👉 交易样本量: {len(trades)} 笔 (统计充分!)")
    print(f"  👉 胜率 (Win Rate): {wr:.2f}% (>= 51.0%)")
    print(f"  👉 盈亏比 (Profit Factor): {pf:.2f}")
    print(f"  👉 净利润: {net:+,.2f} 元")
    print(f"  👉 起始交易时间: {trades[0]['dt']}")
    print(f"  👉 结束交易时间: {trades[-1]['dt']}")
    print("=" * 80)
