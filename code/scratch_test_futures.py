"""
Test script for verifying 15m futures high profit/loss ratio framework.
"""
import sqlite3
import pandas as pd
import numpy as np
...
print("Loading 15m futures data...")
df_15m = load_and_resample_15m_futures(db_path)

# Extract Features
print("Computing alpha features...")
computed_chunks = []
for sym, group in df_15m.groupby("symbol", sort=False):
    ...
    # 8. ATR-14 and Trend Filter (EMA-48)
...
feature_df = pd.concat(computed_chunks, ignore_index=True).sort_values(["trade_time", "symbol"]).reset_index(drop=True)
feature_cols = [
    ...
    "amihud_illiquidity_20", "ret_3", "ret_5", "ret_20",
]
clean_df = feature_df.dropna(subset=feature_cols + ["label", "atr_14", "ema_48"]).copy()
...
def _calc_composite(g):
    ...
    g["score"] = 2.0 * ker + 1.5 * ch + 1.5 * mom + 1.0 * sq + 1.8 * ml
    return g

test_eval_df = test_df.groupby("trade_time", group_keys=False).apply(_calc_composite)

# Backtest Simulation with Asymmetric Trailing Stop
top_k = 3
...
kers = test_eval_df.pivot(index="trade_time", columns="symbol", values="kaufman_efficiency_20")
...
daily_records = []
trade_logs = []
symbol_pnls = {s: [] for s in test_eval_df["symbol"].unique()}
...
for i, t in enumerate(times):
    ...
    # 1. Asymmetric Intra-Bar Stop Loss & Profit Trail
    ...
    for sym in list(long_positions.keys()):
        ...
        # 3-Stage Asymmetric Rules:
        # Stage 1: Hard Stop = Entry - 1.2 * ATR
        # Stage 2: Breakeven Stop = Entry + 0.1 * ATR when Peak >= Entry + 1.5 * ATR
        # Stage 3: Trailing Lock = Peak - 1.0 * ATR when Peak >= Entry + 2.5 * ATR
        hard_stop = entry_p - 1.2 * atr_val
        if peak_p >= entry_p + 2.5 * atr_val:
            effective_stop = max(entry_p + 0.5 * atr_val, peak_p - 1.0 * atr_val)
        elif peak_p >= entry_p + 1.5 * atr_val:
            effective_stop = entry_p + 0.1 * atr_val
        else:
        ...
        if p <= effective_stop:
            ...
            trading_cost += cost
            turnover += notional
            tag = "TRAIL_PROFIT_LONG" if effective_stop > hard_stop else "HARD_STOP_LONG"
    ...
    for sym in list(short_positions.keys()):
        ...
        # 3-Stage Asymmetric Rules for Short:
        hard_stop = entry_p + 1.2 * atr_val
        if trough_p <= entry_p - 2.5 * atr_val:
            effective_stop = min(entry_p - 0.5 * atr_val, trough_p + 1.0 * atr_val)
        elif trough_p <= entry_p - 1.5 * atr_val:
            effective_stop = entry_p - 0.1 * atr_val
        else:
            effective_stop = hard_stop
        ...
        if p >= effective_stop:
            ...
            trading_cost += cost
            turnover += notional
            tag = "TRAIL_PROFIT_SHORT" if effective_stop < hard_stop else "HARD_STOP_SHORT"
            trade_logs.append({"time": t, "symbol": sym, "side": tag, "pnl": pnl, "cost": cost})
            symbol_pnls[sym].append(pnl)
            del short_positions[sym]

    # Scheduled Entry with Controlled Margin (Max 8% equity per position, total 48% margin)
    ...
    if is_rebalance_bar and len(current_scores) >= (top_k * 2 + 4):
        long_candidates = [s for s in current_scores.nlargest(top_k * 3).index if current_kers.get(s, 0.0) >= 0.22]
        buy_longs = long_candidates[:top_k]

        short_candidates = [s for s in current_scores.nsmallest(top_k * 3).index if current_kers.get(s, 0.0) >= 0.22]
        buy_shorts = short_candidates[:top_k]

        max_margin_per_leg = total_equity * 0.07  # 7% margin per position
        risk_cny_target = total_equity * 0.008   # 0.8% portfolio risk

        for sym in buy_longs:
            ...
            mult = spec["multiplier"]
            atr_val = current_atrs.get(sym, p * 0.008)
            risk_per_lot = mult * 1.2 * atr_val
            lots_risk = int(risk_cny_target / max(1e-4, risk_per_lot))
            margin_per_lot = p * mult * spec["margin"]
            lots_margin = int(max_margin_per_leg / max(1e-4, margin_per_lot))
