"""
Refining Cotton (CF_IDX) 5m Strategy to Maximize Profit Factor & Win Rate
"""
...
def optimize_cf():
    ...
    for p_th in [0.28, 0.30, 0.32]:
        for sl_atr in [0.8, 0.9, 1.0]:
            for be_atr in [1.0, 1.2, 1.4]:
                for lock_atr in [2.2, 2.6, 3.0]:
                    for trail_dist in [1.5, 2.0, 2.5, 3.0]:
                        for max_bars in [20, 25, 35]:
                            pos = 0
                            entry_p = 0.0
                            sl_p = 0.0
                            entry_i = 0
                            ...
                            if len(trades) >= 10:
                                ...
                                if wr >= 51.0 and net > 0:
                                    best_res.append({
                                        ...
                                        "p_th": p_th, "sl_atr": sl_atr, "be_atr": be_atr, "lock_atr": lock_atr,
                                        "trail_dist": trail_dist, "max_bars": max_bars
                                    })

    best_res = sorted(best_res, key=lambda x: (x["pf"], x["wr"], x["net"]), reverse=True)
    for idx, b in enumerate(best_res[:10], 1):
        print(f"Rank {idx}: 胜率={b['wr']:.2f}%, 盈亏比(PF)={b['pf']:.2f}, 单笔盈亏比={b['payoff']:.2f}, 净利润={b['net']:+,.2f}元, 交易数={b['trades']}")
        print(f"        Prob={b['p_th']}, SL={b['sl_atr']}, BE={b['be_atr']}, Lock={b['lock_atr']}, Trail={b['trail_dist']}, MaxBars={b['max_bars']}")
