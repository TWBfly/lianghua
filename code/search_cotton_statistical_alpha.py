"""
Statistical Alpha Search for Cotton (CF_IDX) 5m Strategy:
Constraints:
- Trade Count >= 80 (statistically significant, ~1-2 trades/day over 10 months)
- Win Rate >= 51.0%
- Profit Factor >= 3.0 (or highest attainable PF with high win rate)
"""
...
def search_high_sample_alpha(df_feat):
    feature_cols = ["squeeze_5m", "accel_5m", "vol_burst_5m", "donchian_dist_5m", "rsi_14_5m", "oi_flow_5m", "fast_trend", "slow_trend"]
    df_clean = df_feat.dropna(subset=feature_cols + ["atr_14"]).reset_index(drop=True)

    candidates = []

    for horizon in [16, 20, 24, 30]:
        for target_atr in [2.4, 2.8, 3.2, 3.6]:
            for sl_label in [0.8, 1.0]:
                ...
                while current_idx < n_samples:
                    ...
                    clf_l = lgb.LGBMClassifier(n_estimators=60, learning_rate=0.03, max_depth=3, num_leaves=6, random_state=42, verbose=-1, n_jobs=2)
                    clf_s = lgb.LGBMClassifier(n_estimators=60, learning_rate=0.03, max_depth=3, num_leaves=6, random_state=42, verbose=-1, n_jobs=2)
                ...
                for p_th in [0.22, 0.24, 0.25, 0.26]:
                    for sl_atr in [0.7, 0.8, 0.9]:
                        for be_atr in [1.0, 1.2, 1.4]:
                            for lock_atr in [1.8, 2.2, 2.6]:
                                for trail_dist in [1.5, 2.0, 2.5]:
                                    for max_bars in [25, 35, 45]:
                                        pos = 0
                                        entry_p = 0.0
                                        sl_p = 0.0
                                        entry_i = 0
                                        ...
                                        for i in range(len(df_sim)):
                                            ...
                                            if pos != 0:
                                                ...
                                                if pnl_atr >= lock_atr and not half_locked and lots > 1:
                                                    ...
                                                    trades.append({"pnl": realized - fee, "type": "PARTIAL_TP"})
                                                    lots -= close_lots
                                                    half_locked = True
                                                ...
                                                if (hit_sl or timeout) and lots > 0:
                                                    ...
                                                    trades.append({"pnl": realized - fee, "type": "SL" if hit_sl else "TIMEOUT"})
                                                    pos = 0
                                                    lots = 0
                                                    half_locked = False

                                            if pos == 0:
                                                long_cond = sim_pl[i] >= p_th and sim_m30[i] > 0
                                                short_cond = sim_ps[i] >= p_th and sim_m30[i] < 0
                                        ...
                                        if len(trades) >= 80:  # 要求至少 80 笔交易，确保统计有效性
                                            ...
                                            if wr >= 50.0 and net > 0:
                                                candidates.append({
                                                    "wr": wr, "pf": pf, "payoff": payoff, "net": net, "trades": len(tr),
                                                    "horizon": horizon, "target_atr": target_atr, "sl_label": sl_label,
                                                    "p_th": p_th, "sl_atr": sl_atr, "be_atr": be_atr,
                                                    "lock_atr": lock_atr, "trail_dist": trail_dist, "max_bars": max_bars
                                                })

    print(f"✅ 统计充分样本池 (交易数 >= 80 笔 且 胜率 >= 50% 且 净利 > 0): {len(candidates)} 组")
    candidates = sorted(candidates, key=lambda x: (x["wr"] >= 51.0, x["pf"], x["net"]), reverse=True)
    for idx, c in enumerate(candidates[:15], 1):
        print(f"Rank {idx}: 交易数={c['trades']}, 胜率={c['wr']:.2f}%, 盈亏比(PF)={c['pf']:.2f}, 单笔盈亏比={c['payoff']:.2f}, 净利={c['net']:+,.2f}元")
        print(f"        horizon={c['horizon']}, target_atr={c['target_atr']}, prob_th={c['p_th']}, sl_atr={c['sl_atr']}, be_atr={c['be_atr']}, lock_atr={c['lock_atr']}, trail={c['trail_dist']}, max_bars={c['max_bars']}")


if __name__ == "__main__":
    df_raw = load_cotton()
    df_feat = engineer_features(df_raw)
    search_high_sample_alpha(df_feat)
