"""
Clean Robust TianJi-Futures-15M-CSM Execution Engine
Combining:
- 0.90x Gross Notional Neutral Allocation (0.45x Long / 0.45x Short)
- Asymmetric Fast Stop Loss (-1.0%)
- Sector Diversification Guard (Max 2 per Sector)
- 4-Hour (16-Bar) Rebalance Momentum Cycle
"""
import pandas as pd
import numpy as np
import sqlite3
...
for i, t in enumerate(times):
    ...
    # 2. Rebalance (Every 16 bars) with Sector Guard
    is_rebalance_bar = (i % rebalance_bars == 0)
    if is_rebalance_bar and len(current_scores) >= (top_k * 2) * 2:
        # Long targets (max 2 per sector)
        long_cand = current_scores.sort_values(ascending=False).index.tolist()
        long_targets = []
        long_sec_cnt = {}
        for s in long_cand:
            sec = SECTOR_MAP.get(s, "OTHER")
            if long_sec_cnt.get(sec, 0) < 2:
                long_targets.append(s)
                long_sec_cnt[sec] = long_sec_cnt.get(sec, 0) + 1
                if len(long_targets) >= top_k:
                    break

        # Short targets (max 2 per sector)
        short_cand = current_scores.sort_values(ascending=True).index.tolist()
        short_targets = []
        short_sec_cnt = {}
        for s in short_cand:
            sec = SECTOR_MAP.get(s, "OTHER")
            if short_sec_cnt.get(sec, 0) < 2:
                short_targets.append(s)
                short_sec_cnt[sec] = short_sec_cnt.get(sec, 0) + 1
...
print("=" * 65)
print("   TIANJI FUTURES 15M CSM (SECTOR GUARD + ASYM STOP BENCHMARK)")
print("=" * 65)
