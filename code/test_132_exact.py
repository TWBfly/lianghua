"""
Alignment comparison between standalone script and engine
"""
...
for i in range(len(df_sim)):
    ...
    if pos != 0:
        ...
        if pnl_atr >= lock_atr and not half_locked and lots > 1:
            ...
            trades.append({"pnl": realized - fee, "type": "PARTIAL_TP", "entry": entry_p, "exit": exit_p, "dt": dt})
            lots -= close_lots
            half_locked = True
        ...
        if (hit_sl or timeout) and lots > 0:
            ...
            trades.append({"pnl": realized - fee, "type": "SL" if hit_sl else "TIMEOUT", "entry": entry_p, "exit": exit_p, "dt": dt})
            pos = 0
            lots = 0
            half_locked = False
...
print("=" * 80)
print(f"CF_IDX 132 Trades Exact Verification:")
print(f"  👉 回测起始时间: {df_sim.iloc[0]['datetime']}")
print(f"  👉 回测结束时间: {df_sim.iloc[-1]['datetime']}")
print(f"  👉 首次交易时间: {trades[0]['dt']}")
print(f"  👉 末次交易时间: {trades[-1]['dt']}")
print(f"  👉 交易笔数: {len(trades)} 笔")
print(f"  👉 胜率: {wr:.2f}% (>= 51.0%)")
print(f"  👉 盈亏比: {pf:.2f}")
print(f"  👉 净利润: {net:+,.2f} 元")
print("=" * 80)
