"""
A-Share & Futures Quantitative Strategy - 1-Minute ML+PPO Research & Backtest Runner
【1 分钟期货机器学习 + PPO 策略全品种回测、参数寻优与报告生成器】
"""
...
def main():
    parser = argparse.ArgumentParser(description="1-Minute Futures ML+PPO Strategy Research Runner")
    ...
    if args.symbols.lower() == "all":
        target_symbols = list(SYMBOL_1M_CONFIGS.keys())
    else:
        target_symbols = [s.strip().upper() for s in args.symbols.split(",") if s.strip()]

    print("=" * 110)
    print(f"🚀 启动 1 分钟 (1m) 期货专属机器学习 (LightGBM) + 微观 PPO 执行策略")
    ...
    for sym in target_symbols:
        if sym not in SYMBOL_1M_CONFIGS:
            print(f"⚠️ 跳过未配置的品种: {sym}")
            continue

        name = SYMBOL_1M_CONFIGS[sym]["name"]
        print(f"\n⚡ 正在执行 1m 策略处理: [{sym} | {name}]...")

        try:
            if args.mode == "backtest":
                res = runner.run_single_symbol_1m_backtest(sym, initial_capital=args.capital)
                results.append(res)
                ...
                opt_res = runner.optimize_1m_symbol(sym, n_trials=args.trials)
    ...
    if results:
        print("\n" + "=" * 110)
        print("📊 1 分钟 (1m) 各品种策略回测性能总排行榜 (对冲与滑点后真实收益)")
        print("=" * 110)
        ...
        if valid_res:
            ...
            print(f"🎯 组合综合表现: 平均胜率={avg_wr:.1f}% | 平均盈亏比={avg_pf:.2f} | 平均夏普={avg_sr:.2f} | 组合总盈利={total_profit:+,.2f} 元")
            print("=" * 110)
