"""
code/run_taichong_30m_squad_lln_audit.py — 「太冲·弹塑性张量」30分钟战队 (SC, LC, J, AL, TA, SI) 大数定律全景双轨深度回测与五重硬门禁审计
"""
...
DEFAULT_OUTPUT = PROJECT_ROOT / "data" / "reports" / "taichong_30m_squad_lln_20260828"
TARGET_SYMBOLS = ["SC_IDX", "LC_IDX", "J_IDX", "AL_IDX", "TA_IDX", "SI_IDX"]
...
def summarize_simulation(
    ...
    avg_holding_hours = float(trades["holding_bars"].mean() * (30.0 / 60.0)) if count else 0.0
...
def load_real_bars_30m(symbol: str, db_path: Path = DB_PATH) -> pd.DataFrame:
    with sqlite3.connect(db_path) as connection:
        frame = pd.read_sql_query(
            """
            SELECT trade_time, open, high, low, close, volume, open_interest
            FROM futures_min_bars
            WHERE symbol = ? AND timeframe = '30m'
            ORDER BY trade_time
            """,
            connection,
            params=(symbol,),
...
def _stable_seed_30m(symbol: str) -> int:
    return 20_260_830 + sum((index + 1) * ord(char) for index, char in enumerate(symbol))


def _fast_generate_regime_bars_30m(
    symbol: str,
    start_price: float,
    bars_per_regime: int,
    tick_size: float,
...
def generate_until_lln_30m(
    ...
    rng = np.random.default_rng(_stable_seed_30m(symbol))
    ...
    for _ in range(max_batches):
        batch = _fast_generate_regime_bars_30m(
            symbol=symbol,
        ...
        batch.index = pd.date_range(next_time, periods=len(batch), freq="30min")
        batch["regime"] = batch["regime"].astype("category")
        next_time = batch.index[-1] + pd.Timedelta(minutes=30)
    ...
    count = sum(len(item["trades"]) for item in batch_results)
    raise RuntimeError(f"{symbol} (30m) generated only {count} trades after {max_batches} batches")
...
def _attach_trade_context(
    ...
    contextual.insert(0, "timeframe", "30m")
...
def run_single_squad_symbol_30m(
    symbol: str,
    spec: dict[str, Any],
    target: int = 1_001,
    bars_per_regime: int = 10_000,
) -> dict[str, Any]:
    real_bars = load_real_bars_30m(symbol)
    ...
    synthetic_batches, synthetic_signals, synthetic_result = generate_until_lln_30m(
        symbol,
        spec,
        target=target,
        bars_per_regime=bars_per_regime,
    ...
    synthetic_trades.insert(0, "timeframe", "30m")
    ...
    return {
        ...
        "parameter_rows": [dict(symbol=symbol, timeframe="30m", **row) for row in robustness["parameter_rows"]],
    }


def compute_squad_scorecard_30m(summary: dict[str, Any]) -> dict[str, Any]:
    real = summary["real"]
...
def main():
    parser = argparse.ArgumentParser(description="太冲·弹塑性张量 30分钟战队大样本深度回测")
    ...
    print(f"🚀 [TaiChong 30m Squad LLN] 启动 30分钟波段战队 {symbols} 大样本回测与五重硬门禁审计...", flush=True)
    ...
    with ProcessPoolExecutor(max_workers=args.max_workers) as executor:
        ...
        for future in as_completed(futures):
            ...
            try:
                res = future.result()
                results.append(res)
                print(f"✅ [30m Squad] {sym} 回测与审计完成 (真实历史 + 合成大数 >=1000 笔已达成)", flush=True)
            except Exception as e:
                print(f"❌ [30m Squad] {sym} 失败: {e}", flush=True)
                import traceback
                traceback.print_exc()
    ...
    all_metrics.to_csv(output_dir / "squad_30m_metrics.csv", index=False)
    all_trades.to_csv(output_dir / "squad_30m_trades.csv", index=False)
    all_regimes.to_csv(output_dir / "squad_30m_regimes.csv", index=False)
    all_parameters.to_csv(output_dir / "squad_30m_parameters.csv", index=False)
    ...
    scorecard = compute_squad_scorecard_30m(squad_summary)
    ...
    print("📊 [TaiChong 30m Squad LLN] 审计与回测报告已成功输出！", flush=True)
