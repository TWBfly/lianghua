"""
Search for Win Rate >= 51% and Profit Factor >= 3.0 for Cotton (CF_IDX) 5m
"""
...
def search_perfect():
    ...
    # 尝试不同标签配置
    targets = []
    for horizon, target_atr, sl_label in [
        (24, 3.0, 0.8), (30, 3.5, 0.8), (36, 4.0, 0.9), (48, 4.5, 1.0)
    ]:
        ...
        sim_sq = df_sim["squeeze_5m"].values
        sim_vb = df_sim["vol_burst_5m"].values

        # 搜索最佳参数
        for p_th in [0.22, 0.24, 0.26, 0.28, 0.30]:
            for sl_atr in [0.8, 1.0, 1.2]:
                for be_atr in [1.0, 1.2, 1.5, 1.8]:
                    for lock_atr in [1.8, 2.2, 2.6, 3.0]:
                        for trail_dist in [1.2, 1.5, 1.8, 2.2]:
                                ...
                                if len(trades) >= 15:
                                    ...
                                    if wr >= 51.0 and net > 0:
                                        targets.append({
                                            ...
                                            "prob_th": p_th, "sl_atr": sl_atr, "be_atr": be_atr,
                                            "lock_atr": lock_atr, "trail_dist": trail_dist, "max_bars": max_bars
                                        })

    print(f"🎯 符合【胜率 >= 51% 且 净利润 > 0】的策略组合: {len(targets)} 组")
    targets = sorted(targets, key=lambda x: (x["pf"], x["wr"], x["net"]), reverse=True)
    for idx, t in enumerate(targets[:15], 1):
        print(f"Top {idx}: 胜率: {t['wr']:.2f}%, 盈亏比(PF): {t['pf']:.2f}, 单笔盈亏比: {t['payoff']:.2f}, 净利润: {t['net']:+,.2f}元, 交易数: {t['trades']}")
        print(f"        horizon={t['horizon']}, target_atr={t['target_atr']}, prob_th={t['prob_th']}, sl_atr={t['sl_atr']}, be_atr={t['be_atr']}, lock_atr={t['lock_atr']}, trail={t['trail_dist']}, max_bars={t['max_bars']}")
